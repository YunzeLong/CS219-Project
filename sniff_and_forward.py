...
from defines import *
import packet_manipulations as pm


def sniffer_and_filter(packet: Packet):
    # print(packet)
    src_IP = None
    dst_IP = None

    # used in the send phase
    out_ip_packet = None
    ...
    if TCP in packet:
        if IP in packet:
            ...
            out_ip_packet = IP(dest=get_cloud_ip())
        elif IPv6 in packet:
            src_IP = packet[IPv6].src
            dst_IP = packet[IPv6].dst
            out_ip_packet = IPv6(dest=get_cloud_ip6())
        ...
        # TODO: inspect packet content for security

        # send here
        # TODO: test
        tcp_rerouted = pm.reroute_tcp(packet[TCP], get_cloud_port())
        send(out_ip_packet / tcp_rerouted)
...
if __name__ == "__main__":
    ...
    print(
        f"src port: {source_port}, dst port: {destination_port}, dst ip: {destination_ip}"
    )
